...
export function fieldValues(pin: Pin, f: FilterFieldConfig): string[] {
...
export function passesFilters(pin: Pin, filterConfig: FilterFieldConfig[], activeFilters: ActiveFilters): boolean {
...
export function allValuesFor(pins: Pin[], f: FilterFieldConfig): string[] {
  return Array.from(new Set(pins.flatMap((p) => fieldValues(p, f)))).sort((a, b) => a.localeCompare(b));
}

export function initialActiveFilters(pins: Pin[], filterConfig: FilterFieldConfig[]): ActiveFilters {
 ...
 */
export function getValuesForFilter(pins: Pin[], f: FilterFieldConfig, activeFilters: ActiveFilters): string[] {
  let source = pins;
  if (f.dependsOn) {
    const parentActive = activeFilters[f.dependsOn];
    const parentConfig: FilterFieldConfig = { key: f.dependsOn, label: "", sheetColumn: "" };
    if (parentActive) {
...
export function countForValue(pins: Pin[], f: FilterFieldConfig, value: string): number {
